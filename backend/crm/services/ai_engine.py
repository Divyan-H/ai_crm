...
# Upper bound on LLM round-trips per user turn, so a model that keeps
# requesting tools can't loop (and bill) forever.
COPILOT_MAX_STEPS = 8
...
async def copilot_chat(messages: list[dict], org_id: str | None = None) -> AsyncGenerator[dict, None]:
    ...
    for _ in range(COPILOT_MAX_STEPS):
        ...
        # The assistant turn that requested the tools must appear exactly once,
